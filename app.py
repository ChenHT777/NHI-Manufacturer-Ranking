...
if 'html_preview' in st.session_state and 'current_base_filename' in st.session_state:
    ...
    <body>
        ...
        <div id="status-banner" style="display:none; margin-top:15px; padding:12px 15px; border-radius:8px; font-family:'微軟正黑體', sans-serif; font-size:14px; line-height:1.6;"></div>

        <script>
            // 用固定狀態列取代 alert()：alert() 在部分手機瀏覽器的 sandboxed iframe 中會被靜音，
            // 導致使用者完全看不到任何錯誤提示（畫面「一閃就消失」）。
            function showStatus(message, type) {{
                var banner = document.getElementById('status-banner');
                if (!banner) return;
                var colors = {{
                    success: {{ bg: '#E6F4EA', color: '#1E7E34', border: '#B7E1C1' }},
                    error:   {{ bg: '#FDECEA', color: '#C0392B', border: '#F5C6CB' }},
                    info:    {{ bg: '#EAF2FB', color: '#1F497D', border: '#CFE2F3' }}
                }};
            ...
            function downloadImage() {{
                ...
                function showInlineImage(blobUrl) {{
                    ...
                    msg.innerHTML = '📱 <b>請長按下方圖片</b>，選擇「儲存圖片」或「加入照片」即可！';
                    msg.style.color = '#333';
                ...
                function fallbackSave(blob, fileName) {{
                    ...
                    if (isIOS) {{
                        // 【iOS 備援方案】無法使用分享選單時，將圖片直接渲染在網頁上，引導長按儲存
                        showStatus('ℹ️ 已產生圖片，請於下方長按圖片儲存。', 'info');
                        showInlineImage(blobUrl);
                    }} else {{
                ...
                function handleBlob(blob, fileName) {{
                    if (!blob) {{
                        showStatus('❌ 圖片產生失敗（canvas 為空），請稍後再試，或改用電腦版下載。', 'error');
                        return;
                    }}

                    // 👇 手機（含 iPhone 上的 Chrome/Safari）優先呼叫原生「分享」選單，可直接選「儲存圖片」。
                    // 若不支援、被權限政策擋下、或使用者取消，一律自動改用備援方案（絕不會沒有反應）。
                    if (isMobile && navigator.canShare) {{
                        var file, canShareFiles = false;
                        try {{
                            file = new File([blob], fileName, {{ type: 'image/png' }});
                            canShareFiles = navigator.canShare({{ files: [file] }});
                        }} catch (e) {{
                            console.warn('Web Share 檢查失敗，改用備援方案:', e);
                            canShareFiles = false;
                        }}

                        if (canShareFiles) {{
                            navigator.share({{ files: [file], title: fileName }}).then(function() {{
                                showStatus('✅ 已開啟分享選單，請選擇「儲存圖片」。', 'success');
                            }}).catch(function(err) {{
                                console.log('分享失敗或取消，改用備援方案:', err);
                ...
                html2canvas(element, {{
                    ...
                    canvas.toBlob(function(blob) {{ handleBlob(blob, fileName); }}, 'image/png');
                }}).catch(function(error) {{
                    restoreUI();
                    console.error('截圖失敗:', error);
                    showStatus('❌ 截圖產生失敗：' + (error && error.message ? error.message : error), 'error');
                }});
            }}
        </script>
    </body>
